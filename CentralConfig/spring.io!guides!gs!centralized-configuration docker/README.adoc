...
:project_id: gs-centralized-configuration-docker

== Notes
Derived from https://cloud.spring.io/guides/gs/centralized-configuration[Centralized Configuration].
Added Docker compose file.

==== Status
Once the services are started, the *Config Client* won't pick the configuration property until you invoke the refresh endpoint on the *Config Server*:
====
[source,bash]
----
...
A solution might be to add the following to the docker-compose.yml:
====
[source,yaml]
----
  config-client:
    ...
    depends_on:
      config-server:
        # [N]:jpa - Depends on config-server.healthcheck setting.
...
This guide walks you through the process of standing up and consuming configuration. See https://docs.spring.io/spring-boot/docs/2.6.3/reference/html/features.html#features.external-config[2. Externalized Configureation from Core Features]
...
----
include::configuration-service/src/main/java/com/example/configurationservice/ConfigurationServiceApplication.java[]
----
====
...
Add a simple property and value (`message = Hello GitHub!`) to the newly created
...
[source,properties]
----
include::configuration-service/src/main/resources/application.properties[]
----
====
...
`configuration-client/src/main/resources/application.properties`. Note that the `config-server` part of the url comes from the service name defined in the docker-compose.yml. That service name defines the DNS name to be used in the internal network as defined in the same docker-compose.yml.
...
[source,java,tabsize=2]
----
include::configuration-client/src/main/resources/application.properties[]
----
====
...
(such as `@ConfigurationProperties` or `@Value("${PROP_NAME: DEFAULT_VALUE}")` or through the `Environment`
...
[source,java,tabsize=2]
----
include::configuration-client/src/main/java/com/example/configurationclient/ConfigurationClientApplication.java[]
----
====

== Test the Application

You can test the end-to-end result by starting the Config Service first:
====
[source,bash]
----
$ mvn clean package spring-boot:build-image // or via gradle ...
$ gradle clean bootBuildImage
----
====
And then, once it is running, starting the client: 
====
[source,bash]
----
...
----
====
Visit the client app in the browser at `http://localhost:8080/message`. There, you should see `Hello GitHub!` in the response.

NOTE: because of a bug, once the services are running, the client doesn't display the message at the beginning. You need to invoke the `refresh` Spring Boot Actuator endpoint in order to force the client to refresh itself and draw in the new value (via `curl localhost:8080/actuator/refresh -d {} -H "Content-Type: application/json"`).
...
sees the change by visiting `http://localhost:8888/a-bootiful-client/default`. Spring Boot's Actuator  exposes operational endpoints
...
Then you can confirm it worked by visting the `http://localhost:8080/message` endpoint.
